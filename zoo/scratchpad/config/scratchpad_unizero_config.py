...
def main(seed: int = 0) -> None:
    ...
    scratchpad_unizero_config = EasyDict(scratchpad_unizero_config)

    # ------------------------------------------------------------------
    # Create configuration for importing environment and policy modules
    # ------------------------------------------------------------------
    scratchpad_unizero_create_config: Dict[str, Any] = dict(
        env=dict(
            type="scratchpad",
    ...
    scratchpad_unizero_create_config = EasyDict(scratchpad_unizero_create_config)

    # ------------------------------------------------------------------
    # Combine configuration dictionaries and construct an experiment name
    # ------------------------------------------------------------------
    main_config: EasyDict = scratchpad_unizero_config
    create_config: EasyDict = scratchpad_unizero_create_config

    # Construct experiment name containing key parameters
    main_config.exp_name = (
        f"data_lz/data_unizero_scratchpad/bge-base-en-v1.5/{env_id}/uz_gpu_cen{collector_env_num}_rr{replay_ratio}_ms{max_steps}_"
        f"nlayer{num_layers}_embed{embed_dim}_Htrain{num_unroll_steps}_"
